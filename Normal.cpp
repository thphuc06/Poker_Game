...
{                                              // we check this first after check straight to assure that we do not miss any error
    ...
    {
        for (int i = 1; i < 5; i++)
		{ //check all card is the same suit
            if (p.Point[player.cards[i].suit] != p.Point[player.cards[i - 1].suit])
            {
                return false;
            }
        }
...
{                                    
    ...
    sort(b.cards, b.cards + 5, cmp); // ascending
...
pair<int, pair<int, int>> take_pair(Hand a)
{
    ...
    for (auto x : store)
    {
        if (x.second == 3)
        {
            res.first = p.Point[x.first]; //the first int will represent for 3 cards same.
        }
        else if (x.second == 2 || x.second == 4) //this pair will store either 4 cards same at pair.first or 2 pairs of cards in descending order. 
        {
...
// but if all card is the same -> tie
...
void update_player_data(const string player_name, bool win, int hand_type)
{
    ...
    map<int, int> most_win_strategy; // this will store (card_strength, frequency)
    multimap<int, int, greater<int>> most_win_strategy2; //the map will prioritize the largest key at first for easy updateting (this will store frequency,  card_strength)
    int type, cnt;
