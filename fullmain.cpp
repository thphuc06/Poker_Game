...
{   
    ...
    homeButton.setPosition(1380.f, 900.f);
    ...
    homeButtonText.setPosition(1420.f, 910.f);
    ...
    sf::Text PVE;
    PVE.setFont(font);
    PVE.setCharacterSize(45);
    PVE.setFillColor(dark_blue);
    PVE.setString("PVE");
    PVE.setPosition(1100.f, 760.f);
    ...
    leaderboard.setFillColor(sf::Color(135, 206, 235));  // blue 
    leaderboard.setOutlineColor(sf::Color(255, 191, 0)); // yellow->orange
    leaderboard.setOutlineThickness(5.f);               
    ...
    // background for mode4

    sf::Texture mode_4_background;
    mode_4_background.loadFromFile("assets/mode4_background.png");

    sf::Sprite mode4_background(mode_4_background);
    ...
    while (window.isOpen())
    {
            ...
            if (!isGameStarted)
            {
                if (!check_register && !check_change_leaderboard)
                {
                    ...
                    if (event.MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left)
                    {
                        sf::Vector2i mousePos = sf::Mouse::getPosition(window);
                        sf::Vector2f mousePosFl = window.mapPixelToCoords(mousePos);
                        if (button_mode_4.getGlobalBounds().contains(mousePosFl.x, mousePosFl.y))
                        {
                            mode4_check = true;
                            /*check_register = true;*/
                            gameplay = true;
                ...
                else
                {
                    if (checkButton && event.type == sf::Event::MouseButtonPressed && mode1_check)
                    {
                        if (event.mouseButton.button == sf::Mouse::Left)
                        {
                            ...
                            // cout << "Mouse clicked at: " << mousePosFloat.x << ", " << mousePosFloat.y << endl;
        ...
        if (!isGameStarted)
        {
            ...
            else if (check_change_leaderboard)
            {
                ...
                ifstream file_in_new("Leaderboard/leaderboard.txt");
                string name;
        ...
        else if (gameplay)
        {
            if (mode4_check)
            {
                window.draw(mode4_background); // or use a different background if preferred
                ...
                Hand* Hands = dealing_cards(deck, 2); // 2 players: computer and human

                // Display cards
                float cardSpacing = 125.f;
                float cardWidth = 125 * 0.3;    // 125 is card image width
                float cardHeight = 181.5 * 0.3; // 181.5 is card image height

                // Display computer's cards (top)
                for (int i = 0; i < 5; i++)
                {
                    string textFile = "cardpacks/" + Hands[0].cards[i].rank + "_of_" + Hands[0].cards[i].suit + ".png";
                    ...
                    float posX = 600 + (cardWidth + cardSpacing) * i;
                    float posY = 100;
                ...
                // Display player's cards (bottom)
                for (int i = 0; i < 5; i++)
                {
                    string textFile = "cardpacks/" + Hands[1].cards[i].rank + "_of_" + Hands[1].cards[i].suit + ".png";
                    sf::Texture card_pick;
                    if (!card_pick.loadFromFile(textFile))
                    {
                        cout << "Failed to load file: " << textFile << endl;
                ...
                sf::Text computerLabel;
                computerLabel.setFont(font);
                computerLabel.setString("Computer");
                computerLabel.setCharacterSize(40);
                computerLabel.setFillColor(sf::Color::White);
                computerLabel.setPosition(750, 0);
                window.draw(computerLabel);

                sf::Text playerLabel;
                playerLabel.setFont(font);
                playerLabel.setString("You");
                playerLabel.setCharacterSize(40);
                playerLabel.setFillColor(sf::Color::White);
                playerLabel.setPosition(750, 500);
                window.draw(playerLabel);

                // Determine winner
                int winner = determineHandWin(Hands);
                ...
                winnerText.setPosition(800, 400);
                ...
                case 0:
                    ...
                    break;
                case 1:
                    winnerText.setString("Computer wins with " + type_of_card(check_strength(Hands[0])));
                    break;
                case 2:
                    winnerText.setString("You win with " + type_of_card(check_strength(Hands[1])));
                    break;
                }
                ...
                //    if (playAgainButton.getGlobalBounds().contains(mousePosFloat)) {
                //        // Reset game state
                //        delete[] Hands;
                //        isGameStarted = true;
            ...
            if (mode3_check)
            {
                ...
                if (holdemStage == FOLDING)
                {
                    ...
                    if (current_player < 3)
                    {
                        ...
                        {
                            // cout << "Bỏ qua Player " << current_player + 1 << " (đã fold)\n";
                        ...
                        if (current_player < 3)
                        {
                            ...
                            { // Chỉ xử lý khi đang đợi click mới
                                ...
                                cout << "Click" << mousePosFloat.x << ", " << mousePosFloat.y << "\n";

                                if (opt1.getGlobalBounds().contains(mousePosFloat))
                                {
                                    // cout << "Player " << current_player + 1 << " pick\n";
                                ...
                                if (check_hide_card)
                                {
                                    ...
                                    //     case 0: showcard = { false, true, false };  break;
                                    //     case 1: showcard = { false, false, true };  break;
                                    //     case 2: showcard = { true, false, false };  break;
                                    // }

                                    int pos = 1;
                                    while (!activePlayers[(current_player + pos) % 3] && pos < 3)
                                    {
                                        pos++;
                                    }
                                    showcard[current_player] = false;
                                    showcard[(current_player + pos) % 3] = true;
                                    current_player++;
                                }
                            }
                ...
                else if (holdemStage == SHOWDOWN)
                {
                    ...
                    string holdem_result = compareHands(playerHands, activePlayers, communityCards);
            ...
            if (mode1_check)
            {
                ...
                for (int i = 0; i < number_player; i++)
                {
                    ...
                    if ((i + 1) == winner_pos)
                    {
                        ...
                        /*
                        check if tie was called and update player tie
                        The second pair stores the two players who have the highest tie. If the second value is -1, then take winner_pos = first."
                        */
                    {
                        wintype = true;
                    }
